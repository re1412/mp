	...
	function checkDragAndDropAPI(){
		bSupportDragAndDropAPI = false; //swpark
		/*
		try{
			if( !oNavigator.ie ){
    ...
    function html5Upload() {
    	var tempFile,
    	    sUploadURL;
    	sUploadURL= "/smarteditor/upload"; // swpark
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			...
 			sUrl : '/smarteditor/upload', // swpark
 	        sCallback : '/smarteditor/callback',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
...
// 	    		oCustomEvent.stop(); 수행시 bAllowed 가 false이더라도 alert이 수행되지 않음
 	    	...
 	    	success : function(oCustomEvent) {
 	    		...
 	    		setPhotoToEditor(aResult);
 	    		//console.log(aResult[0].sFileURL);
 	    		//버튼 비활성화
 	    		goReadyMode();
 	    		oFileUploader.reset();
 	    		window.close();
